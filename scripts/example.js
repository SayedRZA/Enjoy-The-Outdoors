...
document.addEventListener("DOMContentLoaded", () => {
    ...
    minimumPrice.addEventListener("change", filterParks)
    maximumPrice.addEventListener("change", filterParks)
    colorSelect.addEventListener("change", filterParks)
...
function filterParks() {
    ...
    {
        filteredParks = nationalParksArray.filter(n => n.category == category);
    }
    ...
    filteredParks = filteredParks.filter(p => p.price >= minimumPrice && p.price <= maximumPrice);

    // color
    if(color !== "Show All"){
        filteredParks = filteredParks.filter(p => p.color == color)
    }

    displayParks(filteredParks);
}

function displayParks(nationalParksArray) {
    ...
    nationalParksArray.forEach(nationalParksArray => {
        ...
        displayParks(nationalParksArray, productsContainer)
        
    });
}

function displayParks(nationalParksArray, parentDiv) {
    // create a div for each product
    const productDiv = document.createElement("div");
    productDiv.classList.add("nationalParksArray") ;
    productDiv.id = "nationalParksArray-" + nationalParksArray.LocationID ;
    // add product to the container
    parentDiv.appendChild(productDiv);

    addProductHeader(nationalParksArray, productDiv);
    addImageRow(nationalParksArray, productDiv);
    addDescription(nationalParksArray, productDiv);
    addCartButton(nationalParksArray, productDiv);
}
